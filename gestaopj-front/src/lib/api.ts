...
if (typeof window !== "undefined") {
  ...
  console.log(
    "🔗 NEXT_PUBLIC_API_URL da env:",
    process.env.NEXT_PUBLIC_API_URL || "NÃO DEFINIDA"
  );
  if (!process.env.NEXT_PUBLIC_API_URL) {
    console.error(
      "❌ ERRO: NEXT_PUBLIC_API_URL não está configurada! Configure no Vercel ou .env.local"
    );
    console.error(
      "❌ Usando fallback localhost (não funcionará em produção):",
      API_BASE_URL
    );
  }
}
