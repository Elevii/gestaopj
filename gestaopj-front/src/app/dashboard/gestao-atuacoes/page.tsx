...
import { StatusAtividade, TipoAtuacao, Atuacao } from "@/types";
import {
  exportAtuacoesToExcel,
  exportAtuacoesToPdf,
  type AtuacaoColumn,
  COLUMN_LABELS,
} from "@/utils/exportAtuacoes";
...
export default function GestaoAtuacoesPage() {
  const router = useRouter();
  const { userCompanies, user: currentUser } = useAuth();
  ...
  const [showExportModal, setShowExportModal] = useState<
    null | "pdf" | "excel"
  >(null);
  const [selectedAtuacaoId, setSelectedAtuacaoId] = useState<string | null>(
    null
  );
  ...
  const [exibirDetalhamentoCompleto, setExibirDetalhamentoCompleto] =
    useState(false);

  // Carregar atuações e membros
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Carregar todas as atuações da empresa
        const allAtuacoes = await atuacaoService.findAll(company.id);
        setAtuacoes(allAtuacoes);
        ...
        const membersData: Array<{ id: string; name: string; email: string }> =
          [];
  ...
  const projetosById = useMemo(() => {
    ...
    for (const p of projetos)
      map.set(p.id, { titulo: p.titulo, empresa: p.empresa });
    return map;
  }, [projetos]);
  ...
  const hdByAtuacaoId = useMemo(() => {
    ...
    for (const a of sorted) {
      const isAtividadeAvulsa = a.atividadeId.startsWith(
        "__ATIVIDADE_AVULSA__"
      );
      ...
      acumuladoPorAtividade.set(
        a.atividadeId,
        acumulado + (a.horasUtilizadas ?? 0)
      );
    }
  ...
  const filteredAtuacoes = useMemo(() => {
    return atuacoes
      .filter((a) =>
        filters.projetoId ? a.projetoId === filters.projetoId : true
      )
      ...
      .filter((a) => {
        ...
        // Se não tem userId na atuação, considerar que não passa no filtro
        if (!a.userId) return false;
        return a.userId === filters.userId;
      })
  ...
  const atuacoesPorMesAno = useMemo(() => {
    ...
    return Array.from(grupos.entries())
      ...
      .map(([chave, atuacoes]) => {
        ...
        const totalHorasUtilizadas = atuacoes.reduce(
          (acc, a) => acc + (a.horasUtilizadas ?? 0),
          0
        );
        return {
          chave,
  ...
  const getAtuacaoById = (id: string): Atuacao | undefined => {
    return atuacoes.find((a) => a.id === id);
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-400">
            Visualize e gerencie todas as atuações dos membros da empresa
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button
            type="button"
            disabled={
              loading || exporting !== null || filteredAtuacoes.length === 0
            }
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
          <EmptyState
            ...
            title="Nenhuma atuação encontrada"
            description="Não há atuações registradas que correspondam aos filtros selecionados."
          />
        </div>
      ) : (
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                      {grupo.atuacoes.length}{" "}
                      {grupo.atuacoes.length === 1 ? "atuação" : "atuações"}
                    </p>
                  </div>
                        ...
                        <div className="relative inline-flex items-center gap-2">
                          ...
                          {tooltipOpen === "HD" && (
                            <div className="absolute z-10 top-full left-0 mt-2 w-64 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 shadow-lg text-xs text-gray-700 dark:text-gray-200">
                              HD = Horas Disponíveis para a atividade antes do
                              registro (HE - HU acumulado).
                              <div className="mt-2">
                                <button
                                  type="button"
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200">
                              {
                                statusLabel[
                                  (a.statusAtividadeNoRegistro ??
                                    "pendente") as StatusAtividade
                                ]
                              }
                            </span>
                          </td>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3 mb-6">
              {(Object.keys(COLUMN_LABELS) as AtuacaoColumn[]).map((column) => {
                ...
                const isDisabled =
                  column === "descricao" || column === "impacto";
            ...
            <div className="mb-6 p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30">
              <label className="flex items-center space-x-3 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setExibirDetalhamentoCompleto(e.target.checked)
                  }
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Se selecionado, o relatório mostrará o detalhamento completo
                    de todas as atuações com todos os dados disponíveis
                    (incluindo descrição e impacto). Caso contrário, mostrará
                    apenas o resumo com as colunas selecionadas.
                  </p>
                </div>
              </label>
            </div>
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button
                ...
                onClick={async () => {
                  if (
                    !exibirDetalhamentoCompleto &&
                    selectedColumns.length === 0
                  ) {
                    alert(
                      "Selecione pelo menos uma coluna ou marque 'Exibir detalhamento Completo'"
                    );
                    return;
                  }
                  ...
                  try {
                    if (showExportModal === "pdf") {
                      await exportAtuacoesToPdf({
                        ...
                        selectedColumns: exibirDetalhamentoCompleto
                          ? undefined
                          : selectedColumns,
                ...
                disabled={
                  !exibirDetalhamentoCompleto && selectedColumns.length === 0
                }
      ...
      {selectedAtuacaoId &&
        (() => {
          ...
          const isAtividadeAvulsa = atuacao.atividadeId.startsWith(
            "__ATIVIDADE_AVULSA__"
          );
          ...
          const hd = isAtividadeAvulsa
            ? 0
            : (hdByAtuacaoId.get(atuacao.id) ?? 0);
